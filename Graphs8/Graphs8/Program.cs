...
internal class Program
{
    ...
    {
        Console.WriteLine("Performed by: Danila Kheylo\n Group: М3О-225Бк-21 \n Task №7 \n Keys: \n -m - The map is read from the adjacency matrix \n -n - start Cell  \n -d - end Cell");
    }
    ...
    {
        return Math.Abs(b.x-a.x) + Math.Abs(b.y-a.y); //h(u,v)= |xu-xv| + |yu-yv|
    }
    static int HeuristicChebyshev(MapCell a, MapCell b)
    {
        return Math.Max(Math.Abs(b.x-a.x), Math.Abs(b.y-a.y)); //h(u,v)= max( |xu-xv|, |yu-yv| )
    }
    static int HeuristicEuclidean(MapCell a, MapCell b)
    {
        return (int)Math.Sqrt(Pow2(b.x - a.x) + Pow2(b.y - a.y)); //h(u,v) = sqrt( (xu-xv)^2 + (yu-yv)^2 )
    }
