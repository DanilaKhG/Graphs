...
namespace Graphs2
{
    ...
    {
        public static void Dfs(IGraph graph, Vertex v, bool[] visited,HashSet<Vertex> newVisited) //Делаем поиск в глубину
        {
            visited[v] = true;
            newVisited.Add(v);
        ...
        public static List<HashSet<Vertex>> Kosaraju(this IGraph graph)
        {
            ...
            var reversed = ReverseGraph(graph); //Инвертируем други орграфа
            ...
            while (stack.Count>0)
            {
                ...
                if (!visited[current])
                {
                    ...
                    Dfs(reversed, current, visited, newVisited); //При запуске DFS помечаем порядок выхода из каждой вершины
                    result.Add(newVisited);
                }
            }
            return result;
        }
        public static IGraph ReverseGraph(IGraph graph)
        {
