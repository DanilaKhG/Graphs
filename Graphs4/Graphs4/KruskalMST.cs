...
namespace Graphs4
{
    public static class Kruskal
    {
        ...
        public static List<(int, int, int)> KruskalMst(this IGraph graph)
        {
            ...
            int e = 0, i = 0;
            var query = edge.OrderBy(t => t.Item3).ToList(); //отсортируем список ребер по неубыванию
            ...
            i = 0;
            while (e<vertexCount-1) //Пока не закончатся вершины
            {
                var nextEdge = (-1,-1,-1);
                nextEdge = query[i];
                ...
                if (x!=y) //Если разные компоненты связности
                {
                    result.Add(nextEdge); //Добавляется ребро в дерево
                    e++;
                    Union(subsets, x, y);
                }
            }
