...
namespace Graph7
{
    public static class Jonson
    {
        ...
        private static (List<int>, bool) BellmanFord(List<(int,int,int)> edges, int s)
        {
            ...
            for (int i = 0; i < s; i++)
            {
                edges.Add(new (s, i, 0)); //Добавляется новая вершина q и рёбра нулевого веса от этой вершины ко всем остальным
            }
            ...
            for (int i = 0; i < s; i++)
            {
                foreach (var edge in edges)
                {
                    ...
                    {
                        dist[edge.Item2] = dist[edge.Item1] + edge.Item3; //вес пути от q до v после алгоритма Б.-Ф.
                    }
                }
            }
        ...
        public static List<List<int>> Johnson(this IGraph graph)
        {
            ...
            if (negativeCycle) //Если Б.-Ф. показал, что сущ. цикл отриц.веса
            {
            ...
            for (int i = 0; i < graph.VertexCount(); i++)
            {
                for (int j = 0; j < graph.VertexCount(); j++)
                {
                    if (origMatrix[i,j] != 0)
                    {
                        matrix[i][j] = origMatrix[i,j] + modifyWeights[i] - modifyWeights[j]; //Веса рёбер пересчитываются, как расстояние в ориг. графе от u до v + вес(u) - вес(v)
                    }
                }
            }
            //После чего для выполняем алгоритм Дейксторы для графа с пересчитанными весами
            List<List<int>> result = new List<List<int>>();

            for (int i = 0; i < graph.VertexCount(); i++)
            {
                result.Add(Dijkstra(origMatrix, matrix, i, modifyWeights)); // d(u,v) = d(u,v) + d(v) - d(u)
            }
            return result;
        }
    }
